...
import {
  ...
  ChevronRight, Star, MapPin, Clock, TrendingUp, Heart,
  Grid3X3, List, SlidersHorizontal, ArrowUpDown, Eye,
  ChevronDown, Play, Award, Target, Zap, Brain
} from "lucide-react";
...
export default function Subjects() {
  ...
  const [sortBy, setSortBy] = useState<'popularity' | 'rating' | 'price' | 'alphabetical'>('popularity');
      ...
      subjects: [
        {
          ...
          averageRating: 4.9,
          popularity: 98,
          level: ['elementary', 'middle-school', 'high-school', 'university', 'adult'],
          description: 'Grammar, vocabulary, conversation, and academic writing',
          trending: false,
          featured: true
        },
        {
      ...
      subjects: [
        {
          ...
          description: 'Python, JavaScript, Java, C++, and more programming languages',
          trending: true,
          featured: true
        },
        {
  ...
  const filteredSubjects = allSubjects.filter(subject => {
    ...
    const matchesLevel = levelFilter === 'all' || subject.level.includes(levelFilter);
    const matchesPrice = subject.averagePrice.min <= priceRange[1] && subject.averagePrice.max >= priceRange[0];
  ...
  const totalTutors = allSubjects.reduce((sum, subject) => sum + subject.tutorCount, 0);

  const renderSubjectCard = (subject: Subject, featured = false) => (
    <Card key={subject.id} className={`hover:shadow-lg transition-all duration-200 cursor-pointer group ${featured ? 'ring-2 ring-primary/20' : ''}`}>
      <CardContent className="p-6">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${
              subjectCategories.find(cat => cat.id === subject.category)?.color || 'bg-gray-500'
            }`}>
        ...
        <div className="space-y-3">
          ...
          <div className="flex justify-between items-center text-sm">
            ...
            <span className="font-semibold">
              {subject.averagePrice.min.toLocaleString()} - {subject.averagePrice.max.toLocaleString()} UZS
            </span>
          </div>
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="hidden md:block">
                <div className="text-right">
                  <div className="text-3xl font-bold text-primary">{allSubjects.length}+</div>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-7xl mx-auto">
          ...
          <section className="mb-12">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-gray-900">Featured Subjects</h2>
              <Button variant="outline">
                View All <ChevronRight className="h-4 w-4 ml-1" />
              </Button>
            </div>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {featuredSubjects.slice(0, 6).map(subject => renderSubjectCard(subject, true))}
            </div>
          </section>
                      ...
                      <div className="space-y-2 mb-4">
                        {category.subjects.slice(0, 3).map((subject) => (
                          <div key={subject.id} className="flex justify-between items-center text-sm">
                            ...
                            <span className="text-gray-500">{subject.tutorCount} tutors</span>
                          </div>
                        ))}
                ...
                <Card>
                  ...
                  <CardContent className="space-y-3">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold text-primary">{filteredSubjects.length}</span>
                    </div>
                  </CardContent>
                </Card>
              </div>
