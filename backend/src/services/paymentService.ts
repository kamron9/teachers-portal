...
export class PaymentService {
  ...
  async processRefund(request: RefundRequest): Promise<RefundResponse> {
    switch (request.provider) {
      ...
      default:
        throw new PaymentError(
          `Unsupported refund provider: ${request.provider}`,
          "UNSUPPORTED_PROVIDER",
        );
    }
  }
  ...
  private async createClickPayment(
    ...
    try {
      ...
      const params = {
        ...
        return_url:
          request.returnUrl || `${config.frontendUrl}/payment/success`,
      ...
      if (response.data.error_code !== 0) {
        throw new Error(
          response.data.error_note || "Click payment creation failed",
        );
      }
      ...
      throw new PaymentError(
        `Click payment failed: ${error.message}`,
  ...
  private async processClickRefund(
    ...
    try {
      const response = await axios.post(
        "https://api.click.uz/v2/merchant/refund",
        {
          ...params,
          sign: signature,
        },
      ...
      throw new PaymentError(
        `Click refund failed: ${error.message}`,
        "CLICK_REFUND_FAILED",
      );
    }
  }
  ...
  private processClickWebhook(payload: any): any {
    const { action, click_trans_id, merchant_trans_id, amount, error } =
      payload;
  ...
  private async createPaymePayment(
    ...
    try {
      ...
      throw new PaymentError(
        `Payme payment failed: ${error.message}`,
        "PAYME_PAYMENT_FAILED",
      );
    }
  }
  ...
  private async processPaymeRefund(
    ...
    try {
      ...
      throw new PaymentError(
        `Payme refund failed: ${error.message}`,
        "PAYME_REFUND_FAILED",
      );
    }
  }
  ...
  private async createUzumBankPayment(
    request: PaymentRequest,
  ): Promise<PaymentResponse> {
    try {
      ...
      throw new PaymentError(
        `Uzum Bank payment failed: ${error.message}`,
        "UZUM_BANK_PAYMENT_FAILED",
      );
    }
  }

  private async processUzumBankRefund(
    request: RefundRequest,
  ): Promise<RefundResponse> {
    // Mock implementation
    return {
  ...
  private processStripeWebhook(payload: any): any {
    return {
      ...
      status:
        payload.type === "payment_intent.succeeded" ? "COMPLETED" : "FAILED",
