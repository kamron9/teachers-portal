...
export const createNotificationSchema = Joi.object({
  ...
  relatedUserId: Joi.string().uuid({ version: "uuidv4" }).optional().messages({
    "string.uuid": "Related user ID must be a valid UUID",
  }),

  scheduledFor: Joi.date().iso().min("now").optional().messages({
  ...
  metadata: Joi.object().max(50).optional().messages({
...
export const updateNotificationSchema = Joi.object({
  ...
  actionTaken: Joi.boolean().optional().messages({
    "boolean.base": "actionTaken must be a boolean value",
  }),

  actionTakenAt: Joi.date().iso().optional().messages({
...
export const notificationQuerySchema = Joi.object({
  ...
  isRead: Joi.string().valid("true", "false").optional().messages({
  ...
  sortBy: Joi.string()
    ...
    .messages({
      "any.only":
        "Sort field must be one of: createdAt, priority, type, isRead",
    }),
  ...
  includeRead: Joi.boolean().default(true).messages({
...
export const bulkNotificationSchema = Joi.object({
  notifications: Joi.array()
    .items(
      Joi.object({
        ...
        metadata: Joi.object().max(20).optional().messages({
  ...
  sendImmediately: Joi.boolean().default(true).messages({
  ...
  scheduledFor: Joi.date()
    ...
    .messages({
      ...
      "date.min": "Scheduled date must be in the future",
      "any.required":
        "Scheduled date is required when sendImmediately is false",
    }),
});
  ...
  preferences: Joi.object({
    ...
    frequency: Joi.object({
      digest: Joi.string()
        ...
        .messages({
          "any.only":
            "Digest frequency must be one of: IMMEDIATE, HOURLY, DAILY, WEEKLY",
        }),
      maxPerDay: Joi.number().integer().min(1).max(100).default(50).messages({
...
export const notificationTemplateSchema = Joi.object({
  ...
  language: Joi.string().valid("en", "uz", "ru").default("en").messages({
...
export const notificationAnalyticsSchema = Joi.object({
  ...
  groupBy: Joi.string()
    ...
    .messages({
      "any.only":
        "Group by must be one of: hour, day, week, month, type, channel, priority",
    }),
  ...
  metrics: Joi.array()
    ...
    .messages({
      ...
      "any.only":
        "Metrics must be one of: sent, delivered, read, clicked, failed",
    }),
