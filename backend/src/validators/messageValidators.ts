...
export const createMessageSchema = Joi.object({
  ...
  replyToId: Joi.string().uuid().optional().messages({
...
export const createThreadSchema = Joi.object({
  ...
  initialMessage: Joi.string().trim().min(1).max(2000).optional().messages({
    "string.min": "Initial message cannot be empty",
    "string.max": "Initial message cannot exceed 2000 characters",
  }),

  subject: Joi.string().trim().min(1).max(200).optional().messages({
...
export const reportMessageSchema = Joi.object({
  reason: Joi.string()
    ...
    .messages({
      "any.only":
        "Reason must be one of: SPAM, HARASSMENT, INAPPROPRIATE_CONTENT, SCAM, VIOLENCE, HATE_SPEECH, OTHER",
...
export const messageQuerySchema = {
  ...
  messagesQuery: Joi.object({
    ...
    senderId: Joi.string().uuid().optional().messages({
      "string.uuid": "Sender ID must be a valid UUID",
    }),

    hasAttachments: Joi.boolean().optional().messages({
...
export const updateMessageSchema = Joi.object({
  ...
  isReported: Joi.boolean().optional().messages({
...
export const bulkMessageActionSchema = Joi.object({
  ...
  action: Joi.string()
    ...
    .messages({
      "any.only":
        "Action must be one of: MARK_READ, MARK_UNREAD, DELETE, ARCHIVE",
...
export const messageStatsSchema = Joi.object({
  ...
  threadId: Joi.string().uuid().optional().messages({
