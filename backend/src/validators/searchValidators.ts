...
export const universalSearchSchema = Joi.object({
  ...
  type: Joi.string()
    ...
    .messages({
      "any.only":
        "Search type must be one of: all, teachers, students, subjects",
    }),
  ...
  sortBy: Joi.string()
    ...
    .messages({
      "any.only":
        "Sort field must be one of: relevance, rating, price, experience, createdAt",
    }),
...
export const teacherSearchSchema = Joi.object({
  ...
  maxRating: Joi.number()
    ...
    .messages({
      ...
      "number.ref":
        "Maximum rating must be greater than or equal to minimum rating",
    }),
  ...
  availability: Joi.string().valid("available", "all").optional().messages({
  ...
  sortBy: Joi.string()
    ...
    .messages({
      "any.only":
        "Sort field must be one of: rating, price, experience, reviews, createdAt",
    }),
...
export const studentSearchSchema = Joi.object({
  ...
  registrationDate: Joi.date().iso().optional().messages({
  ...
  hasBookings: Joi.string().valid("true", "false").optional().messages({
  ...
  sortBy: Joi.string()
    ...
    .messages({
      "any.only":
        "Sort field must be one of: name, email, registrationDate, createdAt",
    }),
...
export const subjectSearchSchema = Joi.object({
  ...
  hasTeachers: Joi.string().valid("true", "false").optional().messages({
...
export const advancedFilterSchema = Joi.object({
  textQuery: Joi.string().trim().min(1).max(200).optional().allow("").messages({
  ...
  teacherFilters: Joi.object({
    ...
    education: Joi.string().trim().min(1).max(200).optional().messages({
  ...
  locationFilters: Joi.object({
    ...
    radius: Joi.number().min(1).max(1000).optional().messages({
  ...
  availabilityFilters: Joi.object({
    requiredSlots: Joi.number().integer().min(1).max(50).optional().messages({
  ...
  ratingFilters: Joi.object({
    min: Joi.number().min(1).max(5).optional().messages({
      "number.min": "Minimum rating must be at least 1",
      "number.max": "Minimum rating must not exceed 5",
    }),
    ...
    minReviews: Joi.number().integer().min(0).optional().messages({
  ...
  sortBy: Joi.string()
    ...
    .messages({
      "any.only":
        "Sort field must be one of: relevance, rating, price, experience, reviews, createdAt",
    }),
...
export const searchSuggestionsSchema = Joi.object({
  ...
  type: Joi.string()
    ...
    .messages({
      "any.only":
        "Suggestion type must be one of: all, teachers, subjects, locations",
    }),
