...
export const revenueReportSchema = Joi.object({
  ...
  paymentMethod: Joi.string()
    ...
    .optional()
    .messages({
      'any.only': 'Payment method must be one of: CARD, BANK_TRANSFER, CLICK, PAYME, UZCARD'
    }),
  ...
  maxAmount: Joi.number()
    ...
    .messages({
      ...
      'number.ref': 'Maximum amount must be greater than or equal to minimum amount'
    })
});
...
export const userAnalyticsSchema = Joi.object({
  ...
  registrationSource: Joi.string()
    ...
    .optional()
    .messages({
      'any.only': 'Registration source must be one of: WEB, MOBILE, REFERRAL, SOCIAL, DIRECT'
    }),
  
  includeLoginActivity: Joi.boolean()
    .default(true)
    .messages({
      'boolean.base': 'Include login activity must be a boolean value'
    }),
  
  includeEngagementMetrics: Joi.boolean()
    .default(true)
    .messages({
...
export const teacherPerformanceSchema = Joi.object({
  ...
  sortBy: Joi.string()
    ...
    .default('rating')
    .messages({
      'any.only': 'Sort by must be one of: rating, earnings, bookings, completionRate, responseTime'
    }),
...
export const bookingAnalyticsSchema = Joi.object({
  ...
  includeStatus: Joi.array()
    .items(Joi.string().valid('PENDING', 'CONFIRMED', 'COMPLETED', 'CANCELLED', 'RESCHEDULED'))
    .min(1)
    .optional()
    .messages({
      'array.min': 'At least one status must be provided',
      'any.only': 'Status must be one of: PENDING, CONFIRMED, COMPLETED, CANCELLED, RESCHEDULED'
    }),
  ...
  maxDuration: Joi.number()
    ...
    .messages({
      ...
      'number.ref': 'Maximum duration must be greater than or equal to minimum duration'
    }),
  
  includeRecurring: Joi.boolean()
    .default(true)
    .messages({
      'boolean.base': 'Include recurring must be a boolean value'
    }),
  
  includeCancellationReasons: Joi.boolean()
    .default(true)
    .messages({
...
export const platformMetricsSchema = Joi.object({
  ...
  metrics: Joi.array()
    ...
    .messages({
      ...
      'any.only': 'Metrics must be one of: users, bookings, revenue, content, engagement, performance, system'
    }),
  ...
  forecastPeriods: Joi.number()
    ...
    .messages({
      ...
      'any.required': 'Forecast periods is required when forecasting is enabled'
    })
});
...
export const customReportSchema = Joi.object({
  ...
  dataSource: Joi.string()
    .valid('users', 'bookings', 'payments', 'reviews', 'teachers', 'students', 'subjects')
    .required()
    .messages({
      'any.only': 'Data source must be one of: users, bookings, payments, reviews, teachers, students, subjects',
...
export const reportScheduleSchema = Joi.object({
  ...
  schedule: Joi.object({
    frequency: Joi.string()
      ...
      .required()
      .messages({
        'any.only': 'Frequency must be one of: hourly, daily, weekly, monthly, quarterly',
