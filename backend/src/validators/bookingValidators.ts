...
export const createBookingSchema = Joi.object({
  ...
  subjectOfferingId: Joi.string().uuid().required().messages({
    "string.uuid": "Subject offering ID must be a valid UUID",
    "any.required": "Subject offering ID is required",
  }),

  startAt: Joi.date().iso().min("now").required().messages({
  ...
  studentTimezone: Joi.string().default("Asia/Tashkent").messages({
...
export const updateBookingSchema = Joi.object({
  ...
  lessonNotes: Joi.string().max(1000).optional().messages({
    "string.max": "Lesson notes cannot exceed 1000 characters",
  }),

  studentAttended: Joi.boolean().optional().messages({
    "boolean.base": "Student attended must be a boolean value",
  }),

  teacherAttended: Joi.boolean().optional().messages({
    "boolean.base": "Teacher attended must be a boolean value",
  }),

  actualStartAt: Joi.date().iso().optional().messages({
...
export const bookingQuerySchema = Joi.object({
  ...
  type: Joi.alternatives()
    .try(
      ...
      Joi.array()
        .items(Joi.string().valid("TRIAL", "SINGLE", "PACKAGE"))
        .unique(),
    )
    .optional()
    .messages({
  ...
  sortBy: Joi.string()
    ...
    .messages({
      "any.only":
        "Sort by must be one of: startAt, endAt, createdAt, status, priceAtBooking",
    }),
...
export const rescheduleBookingSchema = Joi.object({
  newStartAt: Joi.date().iso().min("now").required().messages({
  ...
  notifyOtherParty: Joi.boolean().default(true).messages({
...
export const cancelBookingSchema = Joi.object({
  ...
  requestRefund: Joi.boolean().default(true).messages({
...
export const attendanceSchema = Joi.object({
  studentAttended: Joi.boolean().required().messages({
    "boolean.base": "Student attended must be a boolean value",
    "any.required": "Student attendance status is required",
  }),

  teacherAttended: Joi.boolean().required().messages({
    "boolean.base": "Teacher attended must be a boolean value",
  ...
  homeworkAssigned: Joi.string().max(500).optional().messages({
    "string.max": "Homework assignment cannot exceed 500 characters",
  }),

  nextLessonTopics: Joi.string().max(500).optional().messages({
  ...
  studentProgress: Joi.object({
    ...
    difficultyLevel: Joi.string()
      ...
      .messages({
        "any.only":
          "Difficulty level must be one of: VERY_EASY, EASY, MODERATE, DIFFICULT, VERY_DIFFICULT",
      }),
    ...
    studentEngagement: Joi.string()
      ...
      .messages({
        "any.only":
          "Student engagement must be one of: VERY_LOW, LOW, MODERATE, HIGH, VERY_HIGH",
      }),
  }).optional(),
});
...
export const bulkUpdateBookingsSchema = Joi.object({
  ...
  updates: Joi.object({
    ...
    lessonNotes: Joi.string().max(1000).optional(),
  ...
  notifyParties: Joi.boolean().default(true).messages({
