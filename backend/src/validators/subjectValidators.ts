...
export const subjectOfferingSchema = Joi.object({
  ...
  level: Joi.string()
    ...
    .required()
    .messages({
      'any.only': 'Level must be one of: ALL_LEVELS, BEGINNER, ELEMENTARY, INTERMEDIATE, UPPER_INTERMEDIATE, ADVANCED, INTERMEDIATE_PLUS',
  ...
  icon: Joi.string()
    ...
    .default('BOOK')
    .messages({
      'any.only': 'Icon must be one of: BOOK, BAR_CHART, BRIEFCASE, SPEECH_BUBBLE'
    }),
...
export const subjectSearchSchema = Joi.object({
  ...
  sortBy: Joi.string()
    ...
    .default('relevance')
    .messages({
      'any.only': 'Sort by must be one of: relevance, price, rating, experience, lessons'
    }),
