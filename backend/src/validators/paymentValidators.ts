...
export const paymentQuerySchema = Joi.object({
  status: Joi.alternatives()
    .try(
      Joi.string().valid('PENDING', 'COMPLETED', 'FAILED', 'REFUNDED', 'PARTIALLY_REFUNDED'),
      Joi.array().items(
        Joi.string().valid('PENDING', 'COMPLETED', 'FAILED', 'REFUNDED', 'PARTIALLY_REFUNDED')
      ).unique()
    )
    .optional()
    .messages({
      'any.only': 'Status must be one of: PENDING, COMPLETED, FAILED, REFUNDED, PARTIALLY_REFUNDED',
  ...
  sortBy: Joi.string()
    ...
    .default('createdAt')
    .messages({
      'any.only': 'Sort by must be one of: createdAt, amount, status, capturedAt'
    }),
...
export const payoutRequestSchema = Joi.object({
  amount: Joi.number()
    ...
    .messages({
      ...
      'number.max': 'Payout amount cannot exceed 100,000 UZS (10,000,000 kopeks)',
      'number.integer': 'Amount must be an integer (kopeks)',
  ...
  method: Joi.string()
    ...
    .required()
    .messages({
      'any.only': 'Payout method must be one of: BANK_TRANSFER, CARD, UZUM_BANK, CLICK',
  ...
  cardDetails: Joi.object({
    ...
    brand: Joi.string().valid('VISA', 'MASTERCARD', 'UZCARD', 'HUMO').required(),
