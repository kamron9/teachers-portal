...
export const reviewStatsQuerySchema = Joi.object({
  ...
  groupBy: Joi.string()
    ...
    .default('rating')
    .messages({
      'any.only': 'Group by must be one of: rating, subject, teacher, student, date'
    })
});
...
export const flagReviewSchema = Joi.object({
  reason: Joi.string()
    ...
    .required()
    .messages({
      'any.only': 'Reason must be one of: INAPPROPRIATE_LANGUAGE, SPAM, FALSE_INFORMATION, HARASSMENT, OFFENSIVE_CONTENT, OTHER',
...
export const reviewAnalyticsQuerySchema = Joi.object({
  metric: Joi.string()
    ...
    .required()
    .messages({
      'any.only': 'Metric must be one of: rating_trends, response_rates, teacher_performance, subject_satisfaction, moderation_queue, review_volume',
