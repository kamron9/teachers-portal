...
export const adminUserManagementSchema = Joi.object({
  ...
  search: Joi.string().trim().min(1).max(200).optional().allow("").messages({
  ...
  sortBy: Joi.string()
    ...
    .messages({
      "any.only":
        "Sort field must be one of: createdAt, email, lastLogin, role",
    }),
...
export const adminBulkActionSchema = Joi.object({
  ...
  action: Joi.string()
    ...
    .messages({
      "any.only":
        "Action must be one of: ACTIVATE, DEACTIVATE, DELETE, RESET_PASSWORD, SEND_NOTIFICATION",
  ...
  notificationMessage: Joi.string()
    ...
    .messages({
      ...
      "any.required":
        "Notification message is required when action is SEND_NOTIFICATION",
    }),
});
...
export const adminAnalyticsQuerySchema = Joi.object({
  ...
  metric: Joi.string()
    ...
    .messages({
      "any.only":
        "Metric must be one of: users, bookings, revenue, teachers, students, reviews, growth, retention, engagement",
    }),
  ...
  groupBy: Joi.string()
    ...
    .messages({
      "any.only":
        "Group by must be one of: day, week, month, role, subject, region",
    }),
...
export const adminModerationSchema = Joi.object({
  type: Joi.string()
    ...
    .messages({
      "any.only":
        "Type must be one of: all, reviews, teachers, reports, content",
    }),
  ...
  sortBy: Joi.string()
    ...
    .messages({
      "any.only":
        "Sort field must be one of: createdAt, priority, type, assignedTo",
    }),
...
export const adminAuditSchema = Joi.object({
  ...
  resource: Joi.string()
    ...
    .messages({
      "any.only":
        "Resource must be one of: user, teacher, student, review, booking, payment, system",
    }),

  adminId: Joi.string().uuid({ version: "uuidv4" }).optional().messages({
...
export const adminReportSchema = Joi.object({
  reportType: Joi.string()
    ...
    .messages({
      "any.only":
        "Report type must be one of: user_activity, financial_summary, teacher_performance, student_engagement, system_usage, security_audit, content_moderation",
...
export const adminFeatureFlagSchema = Joi.object({
  flagName: Joi.string()
    ...
    .messages({
      ...
      "string.pattern.base":
        "Feature flag name must start with a letter and contain only letters, numbers, and underscores",
  ...
  rolloutPercentage: Joi.number().min(0).max(100).default(100).messages({
