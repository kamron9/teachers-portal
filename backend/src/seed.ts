...
const SUBJECTS = [
  ...
  },
  {
    name: "Russian",
    category: "LANGUAGES",
    description: "Grammar, Literature, Conversation",
  },
  {
    name: "Uzbek",
    category: "LANGUAGES",
    description: "Native language, Literature, Writing",
  },
  {
    name: "History",
    category: "HUMANITIES",
    description: "World History, Uzbekistan History",
  },
  {
    name: "Geography",
    category: "HUMANITIES",
  ...
  },
  {
    ...
    category: "TEST_PREP",
    description: "IELTS Speaking, Writing, Reading, Listening",
  },
  {
    name: "SAT Preparation",
    category: "TEST_PREP",
    description: "SAT Math, English, Essay Writing",
  },
];
  ...
  for (let i = 0; i < count; i++) {
    ...
    const teacher = await prisma.teacherProfile.create({
      data: {
        ...
        verifiedAt:
          Math.random() > 0.2
            ? getRandomDate(new Date(2023, 6, 1), new Date())
            : null,
    ...
    const teacherSubjects = getRandomElements(
      subjects,
  ...
  for (let i = 0; i < count; i++) {
    ...
    await prisma.studentProfile.create({
      data: {
        ...
        region: getRandomElement(REGIONS),
        country: "Uzbekistan",
        educationLevel: getRandomElement([
          "HIGH_SCHOOL",
          "UNDERGRADUATE",
          "GRADUATE",
          "OTHER",
        ]),
        preferredSubjects: getRandomElements(
          SUBJECTS.map((s) => s.name),
...
async function createBookings(count: number = 200) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    let availabilitySlot = getRandomElement(
      teacher.availabilitySlots.filter((slot) => !slot.isBooked),
    );
    ...
    const totalAmount = Math.round(
      (duration / 60) * subjectOffering.pricePerHour,
    );
    ...
    const booking = await prisma.booking.create({
      data: {
        ...
        status,
        notes:
          Math.random() > 0.7 ? "Looking forward to the lesson!" : undefined,
        cancellationReason:
          status === "CANCELLED"
            ? getRandomElement([
                "STUDENT_CANCELLED",
...
async function createReviews(count: number = 150) {
  ...
  const reviewableBookings = getRandomElements(
    completedBookings,
    Math.min(count, Math.floor(completedBookings.length * 0.75)),
  );
  ...
  for (const booking of reviewableBookings) {
    ...
    await prisma.review.create({
      data: {
        ...
        rating,
        comment:
          Math.random() > 0.2 ? getRandomElement(reviewComments) : undefined,
        isAnonymous,
  ...
  logger.info(
    `Created ${reviewableBookings.length} reviews and updated teacher ratings`,
  );
}
  ...
  for (let i = 0; i < count; i++) {
    ...
    await prisma.notification.create({
      data: {
        ...
        readAt:
          Math.random() > 0.4 ? getRandomDate(createdAt, new Date()) : null,
        createdAt,
      },
    });
  }
