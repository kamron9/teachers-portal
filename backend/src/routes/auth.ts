...
router.post('/register', validateRequest(registerSchema), async (req, res) => {
  ...
  if (existingUser) {
    throw new AppError('User already exists with this email or phone', 409, 'USER_ALREADY_EXISTS');
  }
    ...
    if (role === 'STUDENT') {
      ...
      await tx.teacherProfile.create({
        data: {
          ...
          firstName,
          lastName,
          verificationStatus: config.verification.teacherAutoApproval ? 'APPROVED' : 'PENDING',
        }
      });
    }
  ...
  logger.info('User registered', { 
    ...
    phoneVerificationRequired: config.verification.requirePhoneVerification && !!phone
  });
...
router.post('/resend-email-verification', authMiddleware, async (req: AuthRequest, res) => {
  const user = await prisma.user.findUnique({
...
// Change password
router.post('/change-password', authMiddleware, validateRequest(changePasswordSchema), async (req: AuthRequest, res) => {
