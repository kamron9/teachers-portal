...
router.post(
  ...
  async (req, res) => {
    ...
    if (existingCount >= 12) {
      throw new AppError(
        "Maximum 12 subject offerings allowed per teacher",
        400,
...
router.post(
  ...
  async (req, res) => {
    ...
    if (offerings.length !== offeringIds.length) {
      throw new AppError(
        "Some offerings not found or do not belong to you",
        400,
        "INVALID_OFFERINGS",
      );
    }
...
router.get("/stats", requireRole("ADMIN"), async (req, res) => {
  ...
  res.json({
    ...
    subjectAverages: stats[1].map((s) => ({
      ...
      averagePrice: s._avg.pricePerHour
        ? Math.round(s._avg.pricePerHour / 100)
        : 0,
      offeringCount: s._count.id,
    })),
    priceStatistics: {
      min: stats[2]._min.pricePerHour
        ? Math.round(stats[2]._min.pricePerHour / 100)
        : 0,
      max: stats[2]._max.pricePerHour
        ? Math.round(stats[2]._max.pricePerHour / 100)
        : 0,
      average: stats[2]._avg.pricePerHour
        ? Math.round(stats[2]._avg.pricePerHour / 100)
        : 0,
    },
  });
});
