...
router.get("/", validateQuery(bookingQuerySchema), async (req, res) => {
  ...
  if (userRole === "STUDENT") {
    ...
    throw new AppError(
      "Invalid user role for booking access",
      403,
...
router.post(
  ...
  async (req, res) => {
    ...
    const durationMinutes =
      (parseISO(endAt).getTime() - parseISO(startAt).getTime()) / (1000 * 60);
    const durationHours = durationMinutes / 60;
    const priceAtBooking = Math.round(
      subjectOffering.pricePerHour * durationHours,
    );
...
async function validateBookingRequest(params: {
  ...
  if (!teacher) {
    throw new BookingError(
      "Teacher not found or not verified",
  ...
  if (type === "PACKAGE" && packageId) {
    ...
    if (packageInfo.lessonsRemaining <= 0) {
      throw new BookingError(
        "No lessons remaining in package",
        "PACKAGE_EXHAUSTED",
      );
    }
...
async function sendBookingNotifications(
  ...
  try {
    ...
    const notificationData = {
      ...
      duration:
        Math.round(
          (booking.endAt.getTime() - booking.startAt.getTime()) / (1000 * 60),
        ) + " minutes",
