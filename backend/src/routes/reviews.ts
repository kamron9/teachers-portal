...
router.post(
  ...
  async (req, res) => {
    ...
    if (booking.endAt < thirtyDaysAgo) {
      throw new AppError(
        "Cannot review lessons older than 30 days",
        400,
...
router.put(
  ...
  async (req, res) => {
    ...
    if (review.createdAt < twentyFourHoursAgo) {
      throw new AppError(
        "Cannot update review older than 24 hours",
        400,
        "REVIEW_UPDATE_WINDOW_EXPIRED",
      );
    }

    if (review.status !== "PENDING") {
      throw new AppError(
        "Cannot update review that has been moderated",
        400,
...
router.delete(
  ...
  async (req, res) => {
    ...
    if (review.createdAt < twentyFourHoursAgo) {
      throw new AppError(
        "Cannot delete review older than 24 hours",
        400,
